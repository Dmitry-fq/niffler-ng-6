    ...
    public static WebElementsCondition statBubblesInAnyOrder(@Nonnull Bubble... expectedBubbles) {
        return new WebElementsCondition() {
            ...
            public CheckResult check(Driver driver, List<WebElement> actualElements) {
                ...
                if (expectedBubbles.length != actualElements.size()) {
                    final String message = String.format(
                            "List size mismatch (expected: %s, actual: %s)", expectedBubbles.length, actualElements.size()
                    );
                    return rejected(message, actualElements.toString());
                }

                boolean passed = true;
                final List<Bubble> actualBubbleList = new ArrayList<>();
                for (final WebElement actualElement : actualElements) {
                    final Bubble actualBubble = new Bubble(
                            Color.fromRgba(actualElement.getCssValue("background-color")),
                            actualElement.getText());
                    actualBubbleList.add(actualBubble);
    ...
    public static WebElementsCondition statBubblesContains(@Nonnull Bubble... expectedBubbles) {
        return new WebElementsCondition() {
            ...
            public CheckResult check(Driver driver, List<WebElement> actualElements) {
                ...
                boolean passed = Arrays.stream(expectedBubbles).anyMatch(actualBubbleList::contains);
