        ...
        try {
            return Optional.of(
                    entityManager.createQuery(
                                    """
                                            select c from CategoryEntity
                                            c where c.username =: username and c.name =: name
                                            """,
    ...
    public Optional<SpendEntity> findByUsernameAndDescription(String username, String description) {
        try {
            return Optional.of(
                    entityManager.createQuery(
                                    """
                                            select s from SpendEntity
                                            s where s.username =: username and s.description =: description
                                            """,
                                    SpendEntity.class)
